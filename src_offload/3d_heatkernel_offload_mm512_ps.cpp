...
#pragma offload target(mic:mic_id) nocopy(f2: length(V) alloc_if(0) free_if(0) align(64)) \
                                   ...
                                   signal(f1)
     {
       compute_7pt_stencile_mm512_ps(f2, f1, _mnx, _mny, _mnz, _mc1, _mc2); 
       compute_7pt_stencile_mm512_ps(f1, f2, _mnx, _mny, _mnz, _mc1, _mc2);
     }
  }
...
#pragma offload target(mic:mic_id) nocopy(f2: length(V) alloc_if(0) free_if(0) align(64)) \
                                   nocopy(f1: length(V) alloc_if(0) free_if(0) align(64)) \
                                   signal(f1)
       compute_7pt_stencile_mm512_ps(f2, f1, nx, ny, nz, c1, c2); //transfer constants in this case
 }
