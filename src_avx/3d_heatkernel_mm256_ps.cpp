...
#define USE_AVX2 //better as compiler option
...
//#endif
 ...
 printf("\nComputing heat equation with avx2 instructions in %d threads.\n", omp_get_num_threads());
  ...
  float *f1 = (float *)_mm_malloc(problem_size, 32);
  float *f2 = (float *)_mm_malloc(problem_size, 32);

  assert(f1 != MAP_FAILED);
  assert(f2 != MAP_FAILED);

  float *answer = (float *)_mm_malloc(problem_size, 32);
  float *f_final = NULL;
  ...
  for (int i = 0; i < count; ++i) {
    compute_7pt_stencile_mm256_ps(f2_t, f1_t, nx, ny, nz, c0, c1); 
    float *t = f1_t;
    f1_t    = f2_t;
    f2_t    = t;
  }
  ...
  float elapsed_time = (time_end.tv_sec - time_begin.tv_sec)
  ...
  float thput = (nx * ny * nz) * sizeof(float) * 3.0 * count
