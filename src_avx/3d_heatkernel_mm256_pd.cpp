...
#define USE_AVX2
void compute_7pt_stencile_mm256_pd
(double *out, const double *in, const int Lx, const int Ly, const int Lz, const double C0, const double C1)
{
#include "mm256pd_3d_hk_core.h"
  return;
}
#undef USE_AVX2
...
//#endif
 ...
 printf("\nComputing heat kernel using avx2 instructions in %d threads.\n", omp_get_num_threads());
  ...
  double *f1 = (double *)_mm_malloc(problem_size, 32);
  double *f2 = (double *)_mm_malloc(problem_size, 32);

  assert(f1 != MAP_FAILED);
  assert(f2 != MAP_FAILED);

  double *answer = (double *)_mm_malloc(problem_size, 32);
  double *f_final = NULL;
  ...
  for (int i = 0; i < count; ++i) {
    compute_7pt_stencile_mm256_pd(f2_t, f1_t, nx, ny, nz, c0, c1); 
    double *t = f1_t;
    f1_t    = f2_t;
    f2_t    = t;
  }
